...
df['full_label'] = df['label'] + " | " + df['specialization']

labels = df['full_label'].astype('category').cat.categories
os.makedirs("command_modelv3", exist_ok=True)
np.save("command_modelv3/label_names.npy", labels)

df['label'] = df['full_label'].astype('category').cat.codes    # ← перезаписываем label
num_labels = df['label'].nunique()                             # ← теперь 32
...
trainer.save_model("command_modelv3")
tokenizer.save_pretrained("command_modelv3")
